...
- **Real B2B data grounding** — **Orange Slice** (LinkedIn DB: 1.15B profiles / 85M companies) resolves a named business → its real decision-maker → their real LinkedIn URL; **Apify** scrapes reviews. The AI then reports its findings conversationally and grounds the buyer's personality + objections in real data. Graceful fallback to model knowledge when a lookup misses.
...
| B2B data | **Orange Slice** (LinkedIn graph) + **Fiber** + **Apify** (web/review scraping) |
...
- **Everything is one provider-swap away.** LLM calls route through a single `llm()` resolver; sponsors (Composio, Orange Slice, Fiber, Apify, ElevenLabs) each sit behind one small action, so capabilities compose instead of fighting.
...
# Composio / Orange Slice / Apify keys are optional — features degrade gracefully without them
...
Convex (backend + "Best use of Convex"), Cerebras (inference), ElevenLabs (voice), Orange Slice / Fiber (B2B data), Composio (integrations), Apify (scraping), OpenAI (fallback model).
